...
function operator(proxies, targetPlatform) {
    // 1. 确保 HomeSS 节点存在
    const hasHomeSS = proxies.some(p => p.name === "HomeSS");
    
    // 如果没有找到 HomeSS，为了防止策略组报错，可以根据需要决定是否继续
    const backHomeProxies = hasHomeSS ? ["HomeSS", "DIRECT"] : ["DIRECT"];

    // 2. 定义要添加的策略组
    const backHomeGroup = {
        name: "BackHome",
        type: "select",
        proxies: backHomeProxies
    };

    // 3. 定义 IP 规则
    // 注意：192.168.50.9/24 自动指向 192.168.50.0 整个网段
    const backHomeRule = "IP-CIDR,192.168.50.0/24,BackHome,no-resolve";

    // 4. 根据目标平台（Clash, Surge, Stash 等）处理配置
    // Sub-Store 在处理 Script 操作时，通常会通过返回对象的特定字段来插入策略和规则
    return {
        proxies: proxies,
        "policy-groups": [backHomeGroup],
        "rules": [backHomeRule]
    };
}
